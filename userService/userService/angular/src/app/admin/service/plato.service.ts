...
export class PlatoService {
  ...
  listarTodos(): Observable<PlatoResponse[]> {
    return this.http.get<PlatoResponse[]>(this.apiUrl)
      .pipe(catchError(this.handleError));
  }

  listarActivos(): Observable<PlatoResponse[]> {
    return this.http.get<PlatoResponse[]>(`${this.apiUrl}/activos`)
      .pipe(catchError(this.handleError));
  }

  listarActivosOrdenados(): Observable<PlatoResponse[]> {
    return this.http.get<PlatoResponse[]>(`${this.apiUrl}/ordenados`)
      .pipe(catchError(this.handleError));
  }

  listarPorTipo(tipoPlato: string): Observable<PlatoResponse[]> {
    return this.http.get<PlatoResponse[]>(`${this.apiUrl}/tipo/${tipoPlato}`)
      .pipe(catchError(this.handleError));
  }

  buscarPorId(id: number): Observable<PlatoResponse> {
    return this.http.get<PlatoResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  crear(plato: PlatoRequest): Observable<PlatoResponse> {
    return this.http.post<PlatoResponse>(this.apiUrl, plato)
      .pipe(catchError(this.handleError));
  }

  actualizar(id: number, plato: PlatoRequest): Observable<PlatoResponse> {
    return this.http.put<PlatoResponse>(`${this.apiUrl}/${id}`, plato)
      .pipe(catchError(this.handleError));
  }
  ...
  listarEtiquetas(): Observable<EtiquetaResponse[]> {
    return this.http.get<EtiquetaResponse[]>(`${this.etiquetaUrl}/activas`)
      .pipe(catchError(this.handleError));
  }

  crearEtiqueta(etiqueta: EtiquetaRequest): Observable<EtiquetaResponse> {
    return this.http.post<EtiquetaResponse>(this.etiquetaUrl, etiqueta)
      .pipe(catchError(this.handleError));
  }

  actualizarEtiqueta(id: number, etiqueta: EtiquetaRequest): Observable<EtiquetaResponse> {
    return this.http.put<EtiquetaResponse>(`${this.etiquetaUrl}/${id}`, etiqueta)
      .pipe(catchError(this.handleError));
  }

  eliminarEtiqueta(id: number): Observable<void> {
    return this.http.delete<void>(`${this.etiquetaUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }
  ...
  asignarEtiquetas(request: PlatoEtiquetaRequest): Observable<void> {
    return this.http.post<void>(this.platoEtiquetaUrl, request)
      .pipe(catchError(this.handleError));
  }
