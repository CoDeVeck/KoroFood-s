...
public interface IEventoMesaRepository extends JpaRepository<EventoMesa,Integer> {
    ...
    @Query("SELECT COUNT(em) > 0 FROM EventoMesa em WHERE em.idMesa = :idMesa " +
           "AND em.activo = true " +
           "AND em.idEventoMesa != :idEventoMesa " +
           "AND ((em.fechaDesde <= :fechaHasta AND em.fechaHasta >= :fechaDesde))")
    boolean existeSolapamientoMesa(
        @Param("idMesa") Integer idMesa,
        @Param("fechaDesde") LocalDateTime fechaDesde,
        @Param("fechaHasta") LocalDateTime fechaHasta,
        @Param("idEventoMesa") Integer idEventoMesa
    );

    @Query("SELECT COUNT(em) > 0 FROM EventoMesa em WHERE em.idMesa = :idMesa " +
           "AND em.activo = true " +
           "AND ((em.fechaDesde <= :fechaHasta AND em.fechaHasta >= :fechaDesde))")
    boolean existeSolapamientoMesaNuevo(
        @Param("idMesa") Integer idMesa,
        @Param("fechaDesde") LocalDateTime fechaDesde,
        @Param("fechaHasta") LocalDateTime fechaHasta
    );
